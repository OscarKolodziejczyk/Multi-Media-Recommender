...
class MyTestCase(unittest.TestCase):
    ...
    def test_env_vars(self):
        ...
        self.assertIsNotNone(AZURE_CONNECTION_STRING)
        self.assertIsNotNone(CONTAINER_NAME)
        self.assertNotEqual(AZURE_CONNECTION_STRING, "")
        self.assertNotEqual(CONTAINER_NAME, "")

    @patch('ETL.extract_data.BlobServiceClient')
    def test_raw_files_in_dataframes(self, mock_blob_service_client):
        ...
        self.assertIn("raw_movies.csv", dfs.keys())
        self.assertIsInstance(dfs["raw_movies.csv"], pd.DataFrame)
        self.assertEqual(dfs["raw_movies.csv"].iloc[0]['title'], 'Fake Title')
